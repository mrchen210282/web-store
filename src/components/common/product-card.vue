  ...
  <van-cell-group class="additional">
    ...
    >
      ...
      <template slot="tags">
        ...
        <van-stepper v-if="iscard && !isstr" v-model="product.quantity" :max="product.max"  :min="product.min" />
        <van-stepper v-if="iscard && isstr" v-model="product.quantity" :max="1"  :min="1" disabled />
      </template>
    </van-card>
