<template>    
		<view class="white"> 	
			...
			<image src="../../static/zhuce2.png" class="login-logo"></image>
				<view class="clearfix">
					...
					<view class="mui-input-line mt20">
							...
							<input id='password' type="password"  class="mui-input"  v-model="password" placeholder="请输入密码">
						</view>
					<view class="mui-input-line mt20 mb40">
							<view class="icos-lock"></view>
							<input id='password_confirm' type="password" class="mui-input"  v-model="passwordtwo" placeholder="请确认密码">						
					</view>
				
					<view class="btn-next btn " @tap="primaryHandler" >完成</view>
				
				<view class="checkbox-text"><input type="checkbox" checked="checked" class="checked"> 点击完成即表示同意<view style="color: #ff9800; display: inline-block;">《用户协议》</view></view>
			</view>
			</view>
</template>
...
//		components: {uniCountdown},
		...
		methods: {	
			...
			primaryHandler: function() {
				uni.request({
					url: helper.loginUrl+'/api/user/register', //仅为示例，并非真实接口地址。
					method:'POST',
					data: {
							'phone': this.phone,
